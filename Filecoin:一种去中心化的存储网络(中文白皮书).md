...
*（有效询价单）* 从存储矿工发出的询价单M<sub>i</sub>，O<sub>ask</sub>:= (hspace, pricei)M<sub>i</sub>，如果满足下面的条件就是有效的：
...
- 全节点验证：如果节点拥有完整的区块链记录，则可以从创始块开始运行网络协议直到当前区块，这个过程中验证每一个分配给M<sub>i</sub>的“时空证明”。
